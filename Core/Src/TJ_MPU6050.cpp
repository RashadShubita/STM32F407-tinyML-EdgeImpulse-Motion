...
Last Modified: 	26 February 2021 by Rashad Shubita
...
static int16_t MaxAccNoiseThresholdValue_X=100;
static int16_t MinAccNoiseThresholdValue_X=-50;
static int16_t MaxAccNoiseThresholdValue_Y=100;
static int16_t MinAccNoiseThresholdValue_Y=-50;
static int16_t MaxAccNoiseThresholdValue_Z=1021;
static int16_t MinAccNoiseThresholdValue_Z=941;

ScaledData_Def myTempAccelScaled, myTempGyroScaled;
...
void MPU6050_Get_Accel_Scale(ScaledData_Def *scaledDef)
{
    ...
    /* For X-axis */
	if(scaledDef->x > MaxAccNoiseThresholdValue_X || scaledDef->x < MinAccNoiseThresholdValue_X)
	{
		if(scaledDef->x > MaxAccNoiseThresholdValue_X)
		{
			scaledDef->x -= MaxAccNoiseThresholdValue_X;
		}
		else
		{
			scaledDef->x -= MinAccNoiseThresholdValue_X;
		}
	}
	else
	{
    ...
    /* For Y-axis */
	if(scaledDef->y > MaxAccNoiseThresholdValue_Y || scaledDef->y < MinAccNoiseThresholdValue_Y)
	{
		if(scaledDef->y > MaxAccNoiseThresholdValue_Y)
		{
			scaledDef->y -= MaxAccNoiseThresholdValue_Y;
		}
		else
		{
			scaledDef->y -= MinAccNoiseThresholdValue_Y;
		}
	}
	else
	{
    ...
    /* For Z-axis */
	if(scaledDef->z > MaxAccNoiseThresholdValue_Z || scaledDef->z < MinAccNoiseThresholdValue_Z)
	{
		if(scaledDef->z > MaxAccNoiseThresholdValue_Z)
		{
			scaledDef->z -= (MaxAccNoiseThresholdValue_Z-981);
		}
		else
		{
			scaledDef->z += (981-MinAccNoiseThresholdValue_Z);
		}
	}
	else
	{
...
void MPU6050_SetAccNoiseThreshValues(uint16_t NumOfIteration)
{
while(NumOfIteration)
 {
	/* read Accel & Gyro data */
	MPU6050_Get_Accel_Scale(&myTempAccelScaled);
	MPU6050_Get_Gyro_Scale(&myTempGyroScaled);
    /* for X-axis */
	if(myTempAccelScaled.x > MaxAccNoiseThresholdValue_X)
	  {
		MaxAccNoiseThresholdValue_X = myTempAccelScaled.x;
	  }
	else
	  {
		/* do noting */
	  }
	if(myTempAccelScaled.x < MinAccNoiseThresholdValue_X)
	 {
		MinAccNoiseThresholdValue_X = myTempAccelScaled.x;
	 }
	else
	 {
		/* do noting */
    ...
    /* for Y-axis */
	if(myTempAccelScaled.y > MaxAccNoiseThresholdValue_Y)
	  {
		MaxAccNoiseThresholdValue_Y = myTempAccelScaled.y;
	  }
	else
	  {
		/* do noting */
	  }
	if(myTempAccelScaled.y < MinAccNoiseThresholdValue_Y)
	 {
		MinAccNoiseThresholdValue_Y = myTempAccelScaled.y;
	 }
	else
	 {
		/* do noting */
    ...
    /* for Z-axis */
	if(myTempAccelScaled.z > MaxAccNoiseThresholdValue_Z)
	  {
		MaxAccNoiseThresholdValue_Z = myTempAccelScaled.z;
	  }
	else
	  {
		/* do noting */
	  }
	if(myTempAccelScaled.z < MinAccNoiseThresholdValue_Z)
	 {
		MinAccNoiseThresholdValue_Z = myTempAccelScaled.z;
	 }
	else
	 {
		/* do noting */
